...
export async function createProject(source: any, name: string, idemKey?: string) {
  ...
  if ('git' in source) {
    if (!config.allowNetwork) throw { statusCode: 501, message: 'Network not allowed' };
    ...
    try {
      ...
      if (files.length > 0) {
        throw { statusCode: 409, message: 'Project directory already exists and is not empty' };
      }
    } catch {
    ...
    await new Promise((resolve, reject) => {
      exec(gitCmd, (error, stdout, stderr) => {
        if (error) reject({ statusCode: 500, message: 'Git clone failed', details: stderr || error.message });
        else resolve(true);
      });
    });
  } else if ('archiveUrl' in source) {
    throw { statusCode: 501, message: 'Archive download not implemented' };
  } else if ('local' in source || 'adopt' in source) {
    rootAbsPath = path.join(projectsRoot, projectId);
    try {
      await fs.access(rootAbsPath);
    } catch {
  ...
  return { project_id: projectId, root: rootAbsPath };
}
...
export async function renameProject(projectId: string, newName: string) {
  ...
  if (project.rootAbsPath.startsWith(projectsRoot + path.sep)) {
    ...
    try {
      ...
      throw { statusCode: 500, message: 'Failed to rename folder', details: (e as any).message };
    }
  }
  ...
  if (newProjectId !== projectId) await fs.rm(metaPath);
...
export async function deleteProject(projectId: string) {
  ...
  try {
    ...
    throw { statusCode: 500, message: 'Failed to remove project directory', details: (e as any).message };
  }

  // Remove the project metadata file
  try {
    await fs.rm(metaPath);
  } catch (e) {
    throw { statusCode: 500, message: 'Failed to remove project metadata', details: (e as any).message };
  }
